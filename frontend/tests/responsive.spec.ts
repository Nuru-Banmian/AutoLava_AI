...
const longStoreName =
  "特别特别特别特别特别特别特别特别特别特别特别特别特别特别特别特别长的门店名称";
...
async function mockResponsiveApi(
  page: Page,
  { washCountEnabled = true }: { washCountEnabled?: boolean } = {},
) {
  ...
  await page.route(/^http:\/\/127\.0\.0\.1:4173\/api\//, async (route) => {
    ...
    if (path === "/api/database/1/records") {
      ...
      if (pageNumber !== 1 || pageSize !== 200)
        return json({ detail: "page_size must be 200" }, 400);
      return json({
        items: records,
        categories,
...
async function expectNativeMonthInput(
...
test("desktop record and analysis workspaces share the viewport without outer scrolling", async ({
  page,
}) => {
  ...
  expect(
    Math.abs(recordBox!.y + recordBox!.height - analysisBox!.y - analysisBox!.height),
  ...
  await expect
    .poll(() =>
      analysisWorkspace.evaluate((node) => ({
  ...
  await expect
    .poll(async () => {
      ...
      return analysisWorkspace.evaluate(
        (node, bottomGap) => ({
...
test("global store picker switches cleanly between mobile and desktop without header overflow", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
  await mockResponsiveApi(page);
  ...
  await expect(
    page.getByTestId("mobile-store-picker").getByRole("combobox", { name: "门店" }),
  ).toBeVisible();
  ...
  const [pickerBox, brandBox] = await Promise.all([
    desktopPicker.boundingBox(),
    brand.boundingBox(),
  ]);
...
test("320px record list, bottom sheet, and analysis remain reachable without clipping", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
  ...
  await expect
    .poll(() =>
      sheet
        .getByRole("heading", { name: "2026年7月17日" })
  ...
  await expect(
    restSheet.getByRole("heading", { name: "2026年7月16日" }).locator(".."),
  ).toContainText("休息");
  ...
  await expect(
    earlyCloseSheet.getByRole("heading", { name: "2026年7月15日" }).locator(".."),
  ).toContainText("提前休息");
  ...
  const [paginationBox, analysisBox] = await Promise.all([
  ...
  await page
    .getByRole("region", { name: "收入分类" })
    .getByRole("button", { name: /展开收入分类/ })
    .click();
  await page
    .getByRole("region", { name: "其他数据" })
    .getByRole("button", { name: /展开其他数据/ })
    .click();
...
test("record detail hides a positive wash count when the store setting is disabled", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
  await page.setViewportSize({ width: 1280, height: 900 });
...
test("database desktop keeps the wide analysis rail, compact trend, and accessible custom months", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
  await page.setViewportSize({ width: 1280, height: 900 });
  ...
  await expect
    .poll(() => analysisRail.evaluate((node) => node.getBoundingClientRect().width))
  ...
  await expectNativeMonthInput(page.getByLabel("开始月份", { exact: true }), {
    ariaLabel: "开始月份",
    max: "2026-07",
  });
  await expectNativeMonthInput(page.getByLabel("结束月份", { exact: true }), {
    ariaLabel: "结束月份",
    max: "2026-07",
  });
});

test("database at 390px exposes all custom month inputs without horizontal overflow", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
  await page.setViewportSize({ width: 390, height: 844 });
