...
async function mockSettlementWorkbench(
  ...
  const archivedCompanies = [{ id: 25, name: "Historic Transport", is_active: false }];
  await page.route(/^http:\/\/127\.0\.0\.1:4173\/api\//, async (route) => {
    ...
    if (path === "/api/settlements/1")
      return json({ store_id: 1, store_name: "Berlin", company_settlement_enabled: true });
    ...
    if (companyMatch) {
      ...
      if (method === "POST" && action) {
        source.splice(
          source.findIndex((candidate) => candidate.id === id),
          1,
        );
      ...
      if (method === "DELETE") {
        ...
        owningList.splice(
          owningList.findIndex((candidate) => candidate.id === id),
          1,
        );
...
async function expectWorkbenchOrder(page: Page) {
  ...
  expect(
    boxes.slice(0, -1).every((box, index) => box!.y + box!.height <= boxes[index + 1]!.y + 1),
  ).toBe(true);
}
...
async function expectRecordRowsUseAtMostTwoLines(rows: Locator) {
  for (const row of await rows.all()) {
    const cellBoxes = await row
      .locator(":scope > *")
      .evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect().toJSON()));
    expect(
      new Set(cellBoxes.map((box) => Math.round(box.y + box.height / 2))).size,
...
for (const identity of [
  ...
  test(`${identity.label} can enter the same enabled store and read settlement data`, async ({
    page,
  }) => {
    await page.clock.install({ time: new Date("2026-07-21T10:00:00Z") });
    await page.addInitScript(
      ({ userId }) =>
        localStorage.setItem("autolava:selected-store", JSON.stringify({ userId, storeId: 1 })),
    ...
    await expect(
      page.getByRole("navigation", { name: "主导航" }).getByRole("link", { name: "公司结算" }),
    ).toBeVisible();
    await expect(page.getByRole("heading", { name: "公司结算" })).toBeVisible();
    await expect(page.getByLabel("开票月份")).toHaveValue("2026-07");
    await expect(
      page.getByRole("region", { name: "开票记录列表" }).getByText("Beta Logistics"),
    ).toBeVisible();
...
test("1280x900 monthly workbench keeps summaries and record columns aligned", async ({ page }) => {
  ...
  const summaryBoxes = await summaryCards.evaluateAll((nodes) =>
    nodes.map((node) => node.parentElement!.getBoundingClientRect().toJSON()),
  );
  ...
  const registrationBoxes = await Promise.all(
    registrationControls.map((control) => control.boundingBox()),
  );
  ...
  for (const columnIndex of [0, 1, 2, 3]) {
    ...
    const rowBoxes = await rows
      .locator(`:scope > :nth-child(${columnIndex + 1})`)
      .evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect().toJSON()));
  ...
  await expect(
    page.getByRole("button", { name: "确认Alpha Fleet Services开票记录到账" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Alpha Fleet Services开票记录更多操作" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "结算公司管理" })).toHaveAttribute(
    "aria-expanded",
    "false",
  );
...
test("390x844 record and company actions stay compact and keyboard-operable", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "编辑Alpha Fleet Services开票记录" })).toHaveCount(
    0,
  );
  ...
  await expect(
    page.getByRole("menuitem", { name: "编辑Alpha Fleet Services开票记录" }),
  ).toBeFocused();
  await page.keyboard.press("ArrowDown");
  await expect(
    page.getByRole("menuitem", { name: "删除Alpha Fleet Services开票记录" }),
  ).toBeFocused();
  ...
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("menuitem", { name: "撤销Beta Logistics开票记录到账确认" }),
  ).toBeFocused();
  await page.keyboard.press("Escape");
  await expect(confirmedMenu).toBeFocused();
  const pendingColor = await recordsRegion
    .getByText("待到账", { exact: true })
    .evaluate((node) => getComputedStyle(node).backgroundColor);
  const confirmedColor = await recordsRegion
  ...
  await expect
    .poll(() => activeList.evaluate((node) => node.scrollHeight > node.clientHeight))
    .toBe(true);
  ...
  await expect(
    alphaMenu.getByRole("menuitem", { name: "重命名Alpha Fleet Services" }),
  ).toBeFocused();
  await page.keyboard.press("ArrowDown");
  ...
  await activeList.evaluate((node) => {
  ...
  expect(lastCompanyMenuBox!.y + lastCompanyMenuBox!.height).toBeLessThanOrEqual(
    activeListBox!.y + activeListBox!.height,
  );
  await page.keyboard.press("Escape");
  await activeList.evaluate((node) => {
    node.scrollTop = 0;
  ...
  await expect(page.getByRole("tab", { name: "已归档（2）" })).toHaveAttribute(
    "aria-selected",
    "true",
  );
...
test("320px workbench wraps summaries and controls without horizontal overflow", async ({
  page,
}) => {
  ...
  const summaryBoxes = await summaryValues.evaluateAll((nodes) =>
    nodes.map((node) => node.parentElement!.getBoundingClientRect().toJSON()),
  );
