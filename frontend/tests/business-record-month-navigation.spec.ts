...
async function mockBusinessRecords(
  page: Page,
  requests: {
  ...
  await page.route(/^http:\/\/127\.0\.0\.1:4173\/api\//, async (route) => {
    ...
    return route.fulfill({
      status: 404,
      contentType: "application/json",
      body: JSON.stringify({ detail: url.pathname }),
    });
  });
}
...
test("navigates, directly selects, and crosses years without entering a future month", async ({
  page,
}) => {
  ...
  await expectLatestSynchronizedRange(requests, {
    storeId: 1,
    start: "2026-07-01",
    end: "2026-07-31",
  });
...
test("uses month-bounded custom ranges and blocks reversed or future queries at 320px", async ({
  page,
}) => {
  await page.clock.install({ time: new Date("2026-07-17T12:00:00Z") });
...
test("loads every record page for a long custom month range", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      requests.pagedRecords
        .filter((request) => request.start === "2025-01-01" && request.end === "2025-12-31")
