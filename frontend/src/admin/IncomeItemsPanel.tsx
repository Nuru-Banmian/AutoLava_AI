...
function ErrorMessage({ error }: { error: unknown }) {
  ...
  return (
    <p role="alert" className="text-sm text-destructive">
      {friendlyApiError(error, "请求失败，请稍后重试")}
    </p>
  );
}
...
export function IncomeItemsPanel({ storeId, onDirtyChange }: IncomeItemsPanelProps) {
  ...
  async function publishDraft() {
    ...
    try {
      const config = await api<IncomeConfigResponse>(
        `/admin/stores/${capturedStoreId}/income-config`,
        {
          method: "PUT",
          body: JSON.stringify({
            enabled: capturedEnabled,
            items: capturedDraft.map(
              ({ category_id, name, include_in_total, is_active }, sort_order) => ({
                category_id,
                name: name.trim(),
                include_in_total,
                is_active,
  ...
  async function archiveCategory(categoryId: number) {
    ...
    const requestId = beginOperation(capturedStoreId);
    try {
      const category = await api<CategoryWithArchive>(
        `/admin/income-categories/${categoryId}/archive`,
        { method: "POST" },
      );
      if (isCurrentRequest(requestId, capturedStoreId)) {
        setItems((current) =>
  ...
  async function restoreCategory(categoryId: number, capturedStoreId: number) {
    ...
    try {
      const category = await api<CategoryWithArchive>(
        `/admin/income-categories/${categoryId}/restore`,
        { method: "POST" },
      );
      if (isCurrentRequest(requestId, capturedStoreId)) {
        setItems((current) =>
          current.some((item) => item.category_id === category.id)
            ? current
            : [
                ...current,
                {
      ...
      await queryClient.invalidateQueries({
        queryKey: categoriesKey(capturedStoreId),
        exact: true,
      });
      finishOperation(requestId, capturedStoreId);
    } catch (error) {
  ...
  const included = items
    .filter((item) => item.is_active && item.include_in_total && item.name.trim())
  ...
  return (
    <section
      ...
      aria-labelledby="income-items-title"
    >
      <h2 id="income-items-title" className="font-medium">
        收入项目
      </h2>
      ...
      <>
        <label className="flex items-center gap-2">
          <input
            ...
            disabled={operationPending || currentConfig.isLoading || draftStoreId !== storeId}
            type="checkbox"
            onChange={(event) => {
        ...
        <div className="flex flex-col gap-2 rounded-lg border p-4 sm:flex-row">
          <Input
            ...
            placeholder="例如：现金、刷卡"
            value={newName}
            onChange={(event) => setNewName(event.target.value)}
          />
          <Button disabled={operationPending} type="button" variant="outline" onClick={addItem}>
            添加收入项目
          </Button>
        </div>
        {currentConfig.isLoading ? (
          ...
          <ol className="space-y-3">
            {items.map((item, index) => (
              <li
                className="grid gap-3 rounded-lg border p-3 md:grid-cols-[minmax(10rem,1fr)_auto_auto]"
                key={item.key}
              >
                <Input
                  aria-label={`项目名称 ${item.name}`}
                  disabled={operationPending}
                  value={item.name}
                ...
                <label className="flex items-center gap-2">
                  <input
                    aria-label={`计入营业额 ${item.name}`}
                    checked={item.include_in_total}
                    disabled={operationPending}
                    type="checkbox"
                    onChange={(event) =>
                      update(item.key, { include_in_total: event.target.checked })
                    }
                  />
                  计入营业额
                </label>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    disabled={operationPending || index === items.length - 1}
                    type="button"
                    variant="outline"
                    onClick={() => move(index, 1)}
                  >
                  ...
                  {item.category_id !== null ? (
                    <Button
                      ...
                      type="button"
                      variant="outline"
                      onClick={() => void archiveCategory(item.category_id!)}
                    >
                      归档
                    </Button>
                  ) : (
        ...
        {archived.length > 0 && (
          <section className="space-y-2 rounded-lg border p-4" aria-label="已归档收入项目">
            <h2 className="font-medium">已归档项目</h2>
            <ul className="space-y-2">
              {archived.map((category) => (
                <li className="flex flex-wrap items-center justify-between gap-2" key={category.id}>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      type="button"
                      variant="outline"
                      onClick={() => void restoreCategory(category.id, category.store_id)}
                    >
                      恢复
                    </Button>
                    <Button
                      aria-label={`永久删除 ${category.name}`}
                      disabled={operationPending}
                      type="button"
                      variant="outline"
                      onClick={() => {
