...
function latestTimestamp(values: Array<string | null | undefined>) {
  return (
    values
      .map(parseTimestamp)
      .filter((value): value is ParsedTimestamp => value !== null)
...
function formatTimestamp(value: ParsedTimestamp | null, issue: TimestampIssue = null) {
  const issueText =
    issue === "legacy" ? "历史时间时区未知" : issue === "invalid" ? "时间格式缺少时区" : null;
...
export function SystemStatusPanel() {
  ...
  const alerts = useQuery({
    ...
    queryFn: () => api<SystemAlert[]>("/admin/alerts"),
  });
  const taskLogs = useQuery({
    queryKey: ["admin", "task-logs"],
    queryFn: () => api<ScheduledTaskLog[]>("/admin/task-logs"),
  });
  ...
  const dashboardStates = activeStores.map((store, index) => {
    ...
    const utcValues = cards
      .filter((card) => card.timestamp_status === "utc")
      .map((card) => card.generated_at);
    const hasLegacy = cards.some((card) => card.timestamp_status !== "utc");
    const hasInvalid = utcValues.some((value) => parseTimestamp(value) === null);
    return {
      store,
      latest: latestTimestamp(utcValues),
      issue: hasLegacy ? ("legacy" as const) : hasInvalid ? ("invalid" as const) : null,
    };
  });
  ...
  const dashboardIssue = dashboardStates.some((state) => state.issue === "legacy")
    ? "legacy"
    : dashboardStates.some((state) => state.issue === "invalid")
      ? "invalid"
      : null;
  const weatherTasks = (taskLogs.data ?? []).filter((task) => task.task_type === "weather_refresh");
  const weatherTaskTimes = weatherTasks
    .filter((task) => task.timestamp_status === "utc")
    .map((task) => ({
      task,
  ...
  const weatherIssue = weatherTasks.some((task) => task.timestamp_status !== "utc")
    ? "legacy"
    : hasNaiveTimestamp(weatherTaskTimes.map((entry) => entry.raw))
      ? "invalid"
      : null;
  const unresolvedAlerts = (alerts.data ?? []).filter((alert) => !alert.is_resolved);
  const alertTimestampsInvalid = (alerts.data ?? []).some(
    (alert) =>
      alert.timestamp_status !== "utc" ||
      parseTimestamp(alert.created_at) === null ||
      (alert.resolved_at !== null && parseTimestamp(alert.resolved_at) === null),
  );
  const taskTimestampsInvalid = (taskLogs.data ?? []).some(
    (task) =>
      task.timestamp_status !== "utc" ||
  ...
  const hasUnresolvedError = unresolvedAlerts.some(
    (alert) => alert.level.toLowerCase() === "error",
  );
  const latestWeatherFailed =
    latestWeather !== null && latestWeather.task.status.toLowerCase() !== "success";
  const loading =
    stores.isPending ||
    alerts.isPending ||
    taskLogs.isPending ||
    dashboardQueries.some((query) => query.isPending);
  const failed =
    stores.isError ||
    alerts.isError ||
    taskLogs.isError ||
    dashboardQueries.some((query) => query.isError);
  const empty =
    stores.isSuccess &&
    alerts.isSuccess &&
    taskLogs.isSuccess &&
  ...
  const everyStoreHasDashboard =
    activeStores.length > 0 &&
    dashboardStates.every((state) => state.latest !== null && state.issue === null);
  const requiredTimestampsValid =
    !alertTimestampsInvalid && !taskTimestampsInvalid && dashboardIssue === null;
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.7fr)]">
      ...
      >
        <h2 className="font-medium" id="status-summary-title">
          运行状态
        </h2>
        {summary}
        {!loading && !failed && (
          <dl className="grid gap-2 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-muted-foreground">最近天气更新</dt>
              <dd>{formatTimestamp(latestWeather?.parsed ?? null, weatherIssue)}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">最近仪表盘生成</dt>
              <dd>{formatTimestamp(dashboardGeneratedAt, dashboardIssue)}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-muted-foreground">各门店仪表盘</dt>
              <dd>
                <ul>
                  {dashboardStates.map((state) => (
                    <li key={state.store.id}>
                      {state.store.name}：{formatTimestamp(state.latest, state.issue)}
                    </li>
                  ))}
                </ul>
              </dd>
      ...
      {!loading && !failed && (
        <section
          className="space-y-3 rounded-xl border bg-card p-5 shadow-sm"
          aria-labelledby="unresolved-alerts-title"
        >
          <h2 className="font-medium" id="unresolved-alerts-title">
            未解决告警（{unresolvedAlerts.length}）
          </h2>
          {unresolvedAlerts.length === 0 ? (
            ...
            <ul className="space-y-2">
              {unresolvedAlerts.map((item) => (
                <li className="rounded-md bg-muted/40 p-3 text-sm" key={item.id}>
                  <span className="font-medium">{item.level}</span> · {item.message}
                </li>
              ))}
            </ul>
          )}
