...
function formatTimestamp(value: ParsedTimestamp | null, hasNaive = false) {
  if (!value) return hasNaive ? "时间格式缺少时区" : "暂无记录";
...
export function SystemStatusPanel() {
  ...
  const dashboardStates = activeStores.map((store, index) => {
    const values = (dashboardQueries[index]?.data ?? []).map((card) => card.generated_at);
    return { store, latest: latestTimestamp(values), hasNaive: hasNaiveTimestamp(values) };
  });
  ...
  const dashboardHasNaive = dashboardStates.some((state) => state.hasNaive);
  const weatherTasks = (taskLogs.data ?? []).filter((task) => task.task_type.toLowerCase().includes("weather"));
  const weatherTaskTimes = weatherTasks.map((task) => ({
    task,
  ...
  const weatherHasNaive = hasNaiveTimestamp(weatherTaskTimes.map((entry) => entry.raw));
  ...
  const everyStoreHasDashboard = activeStores.length > 0 && dashboardStates.every((state) => state.latest !== null);
  const complete = latestWeather !== null && everyStoreHasDashboard;
  ...
  return <div className="space-y-4">
    <section className="space-y-2 rounded-lg border bg-card p-4" aria-labelledby="status-summary-title">
      ...
      {!loading && !failed && <dl className="grid gap-2 text-sm sm:grid-cols-2">
        <div><dt className="text-muted-foreground">最近天气更新</dt><dd>{formatTimestamp(latestWeather?.parsed ?? null, weatherHasNaive)}</dd></div>
        <div><dt className="text-muted-foreground">最近仪表盘生成</dt><dd>{formatTimestamp(dashboardGeneratedAt, dashboardHasNaive)}</dd></div>
        <div className="sm:col-span-2"><dt className="text-muted-foreground">各门店仪表盘</dt><dd><ul>{dashboardStates.map((state) => <li key={state.store.id}>{state.store.name}：{formatTimestamp(state.latest, state.hasNaive)}</li>)}</ul></dd></div>
      </dl>}
    </section>
