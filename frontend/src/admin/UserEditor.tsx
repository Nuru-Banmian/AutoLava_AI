      ...
      <form className="mt-4 space-y-4" onSubmit={submit}>
        {mode === "create" ? (
          <div className="space-y-1">
            ...
            <Input
              ...
              minLength={3}
              onChange={(event) =>
                update((current) => ({ ...current, username: event.target.value }))
              }
              required
          ...
          <p className="text-sm text-muted-foreground">用户名：{draft.username}</p>
        )}
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            id="user-role"
            onChange={(event) =>
              update((current) => ({ ...current, role: event.target.value as UserRole }))
            }
            value={draft.role}
          >
        ...
        {mode === "edit" && (
          <label className="flex items-center gap-2">
            <input
              ...
              disabled={pending}
              onChange={(event) =>
                update((current) => ({ ...current, is_active: event.target.checked }))
              }
              type="checkbox"
            />
            账号启用
        ...
        {draft.role === "user" && (
          <fieldset className="space-y-2">
            ...
            {assignedUnavailable.map((storeOrId) => {
              ...
              const label =
                typeof storeOrId === "number"
                  ? `未知门店 #${storeOrId}（不可用）`
                  : `${storeOrId.name}（已停用，不可用）`;
              return (
                <label className="flex items-center gap-2 text-muted-foreground" key={id}>
                  <input
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            minLength={8}
            onChange={(event) =>
              update((current) => ({ ...current, password: event.target.value }))
            }
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    此操作不可恢复。确定要永久删除“{user?.username}”吗？
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
