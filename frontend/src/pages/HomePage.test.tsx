...
it("renders the approved missing-yesterday action without assigning ledger state to tomorrow", async () => {
  vi.mocked(useAuth).mockReturnValue({
    user: { id: 2, username: "user", role: "user", is_owner: false },
  ...
  expect(screen.getByRole("link", { name: "补记昨日" })).toHaveAttribute(
    "href",
    "/ledger?date=2026-07-14",
  );
  expect(screen.getByRole("link", { name: "立即记账" })).toHaveAttribute(
    "href",
...
it("keeps cached cards visible and shows the 429 refresh detail", async () => {
  ...
  server.use(
    ...
    http.post("/api/dashboard/1/refresh", () =>
      HttpResponse.json({ detail: "请等待五分钟后再刷新" }, { status: 429 }),
    ),
  );
  ...
  expect(await screen.findByRole("alert")).toHaveTextContent("请等待五分钟");
...
it("clears a refresh error when the selected store changes", async () => {
  ...
  server.use(
    ...
    http.post("/api/dashboard/1/refresh", () =>
      HttpResponse.json({ detail: "wait" }, { status: 429 }),
    ),
  );
  const client = new QueryClient({
    defaultOptions: { queries: { retry: false }, mutations: { retry: false } },
...
it("places the complete Agent workspace before the briefing sidebar for administrators", async () => {
  ...
  server.use(
    http.get("/api/stores/accessible", () =>
      HttpResponse.json([{ id: 1, name: "Roma", timezone: "Europe/Rome" }]),
    ),
