...
import type { DatabaseResponse, IncomeConfigResponse, LedgerBody, RecordSnapshot, WeatherResponse } from "@/api/types";
import { LedgerDatePicker } from "@/components/LedgerDatePicker";
import { LedgerForm } from "@/components/LedgerForm";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from "@/components/ui/alert-dialog";
import { categoryCatalogKey, incomeConfigKey, invalidateUserData, ledgerRecordKey, recentKey, storeLocalToday } from "@/lib/user-api";
...
export function LedgerPage() {
  ...
  return <section className="grid min-w-0 gap-6"><header className="flex min-w-0 flex-wrap items-center justify-between gap-3"><h1 className="text-2xl font-semibold">每日台账</h1><LedgerDatePicker value={date || today} today={today} recordedDates={recordedDates} onChange={setDate} /></header>
    {catalog.isLoading || config.isLoading || record.isLoading ? <p role="status">加载台账…</p> : config.error ? <div role="alert"><span>{friendlyApiError(config.error, "收入配置加载失败，请稍后重试")}</span><button className="ml-2 underline" onClick={() => void config.refetch()}>重试收入配置</button></div> : catalog.error || record.error ? <p role="alert">{(catalog.error ?? record.error)?.message}</p> : <LedgerForm key={`${selected.id}:${date}`} categories={catalog.data?.categories ?? []} config={config.data!} record={record.data ?? undefined} weather={weather.data} saving={save.isPending} onSave={(body) => { setMessage(""); save.mutate({ storeId: selected.id, date, body }); }} />}
