...
function StoreControls() {
  ...
  return (
    <>
      <button onClick={() => select(1)}>choose1</button>
      <button onClick={() => select(2)}>choose2</button>
    </>
  );
}
...
function renderLedger(
  ...
  initialEntry: TestInitialEntry = "/ledger",
) {
  server.use(
    ...extra,
...
describe("LedgerPage", () => {
  it("uses a valid date query parameter for the visible date and API requests", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "选择台账日期：2026年7月14日" }),
    ).toBeInTheDocument();
  ...
  it("rejects a future date query before any date-scoped request is sent", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "选择台账日期：2026年7月15日" }),
    ).toBeInTheDocument();
  ...
  it("never reuses the previous store date while switching across timezone day boundaries", async () => {
    ...
    server.use(
      ...
      http.get("/api/income-config/:store/current", ({ params }) =>
        HttpResponse.json({
          store_id: Number(params.store),
          enabled: false,
          formula: "",
          items: [],
        }),
    ...
    expect(
      await screen.findByRole("button", { name: "选择台账日期：2026年7月16日" }),
    ).toBeInTheDocument();
    expect(await screen.findByRole("button", { name: "保存今日记录" })).toBeEnabled();
  ...
  it("opens the shared calendar and selects a recorded historical date", async () => {
    renderLedger([
      http.get("/api/database/1/records", ({ request }) => {
        const url = new URL(request.url);
        if (url.searchParams.get("page_size") === "200")
          return HttpResponse.json({
            items: [{ id: 7, date: "2026-07-14" }],
            categories: [],
            sum_daily_revenue: 0,
            total: 1,
            page: 1,
    ...
    renderLedger([
      http.get("/api/database/1/records", ({ request }) => {
        if (new URL(request.url).searchParams.get("page_size") === "200")
          return HttpResponse.json({
            items: [{ id: 9, date: "2026-07-14" }],
            categories: [],
            sum_daily_revenue: 101,
            total: 1,
            page: 1,
            page_size: 200,
          });
      ...
      http.get("/api/ledger/1/:date", ({ params }) => {
        ...
        return params.date === "2026-07-14"
          ? HttpResponse.json(historicalRecord)
          : HttpResponse.json({ detail: "not found" }, { status: 404 });
      }),
  ...
  it("reopens the picker with only the selected date month marker request", async () => {
    ...
    renderLedger([
      http.get("/api/database/1/records", ({ request }) => {
        const url = new URL(request.url);
        if (url.searchParams.get("page_size") === "200")
          markerMonths.push(url.searchParams.get("start")!);
        return HttpResponse.json({
          items: [],
          categories: [],
          sum_daily_revenue: 0,
    ...
    const { client } = renderLedger([
      http.get("/api/database/1/records", ({ request }) => {
        if (new URL(request.url).searchParams.get("page_size") === "200") {
          monthRequests += 1;
          return HttpResponse.json({
            items: saved ? [{ id: 7, date: "2026-07-15" }] : [],
            categories: [],
            sum_daily_revenue: 0,
            total: saved ? 1 : 0,
            page: 1,
  ...
  it("returns to Business Records after a successful records-launched save", async () => {
    renderLedger(
      [
        http.put("/api/ledger/1/:date", () =>
          HttpResponse.json({ id: 9, date: "2026-07-15", daily_revenue: 1 }),
        ),
      ],
      {
        pathname: "/ledger",
  ...
  it("keeps a manually selected weather value when delayed automatic weather arrives", async () => {
    const props = {
      config: singleConfig,
      categories: [{ id: 1, name: "现金", include_in_total: true, is_active: true, sort_order: 1 }],
      onSave: () => undefined,
    };
  ...
  it("uses the store timezone rather than browser UTC for today", () => {
    expect(
      storeLocalToday(
        { id: 1, name: "Honolulu", timezone: "Pacific/Honolulu" },
        new Date("2026-07-14T01:00:00Z"),
      ),
    ).toBe("2026-07-13");
    expect(
      storeLocalToday(
        { id: 2, name: "Kiritimati", timezone: "Pacific/Kiritimati" },
        new Date("2026-12-31T10:30:00Z"),
      ),
    ).toBe("2027-01-01");
    expect(
      storeLocalToday(
  ...
  it("renders today and future disabling from the store timezone at a UTC boundary", async () => {
    ...
    renderLedger([
      http.get("/api/stores/accessible", () =>
        HttpResponse.json([{ id: 1, name: "Honolulu", timezone: "Pacific/Honolulu" }]),
      ),
    ]);
  ...
  it("scopes the category catalog to a newly selected historical date", async () => {
    ...
    await waitFor(() =>
      expect(
        requested.some(
          (query) => query.includes("start=2026-06-01") && query.includes("end=2026-06-01"),
        ),
      ).toBe(true),
    );
  });
  ...
  it("confirms before discarding edits when changing the ledger date", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "选择台账日期：2026年7月13日" }),
    ).toBeInTheDocument();
  });
  ...
  it("stays clean after an existing record is saved and refetched canonically", async () => {
    ...
    renderLedger([
      http.get("/api/ledger/1/:date", ({ params }) =>
        params.date === "recent"
          ? HttpResponse.json([])
          : HttpResponse.json(recordSnapshot(saved ? 13 : 12, saved ? "促销" : null)),
      ),
      http.put("/api/ledger/1/:date", async ({ request }) => {
  ...
  it("waits for the post-save record before absorbing delayed automatic weather", async () => {
    ...
    const weatherDelayed = new Promise<void>((resolve) => {
      releaseWeather = resolve;
    });
    let releaseRecord!: () => void;
    const recordDelayed = new Promise<void>((resolve) => {
      releaseRecord = resolve;
    });
    let recordCalls = 0;
    let weatherReturned = false;
    renderLedger([
      http.get("/api/weather/1/:date", async () => {
        await weatherDelayed;
        weatherReturned = true;
        return HttpResponse.json({ weather: "晴" });
      }),
    ...
    renderLedger([
      http.get("/api/ledger/1/:date", ({ params }) => {
        ...
        return recordCalls === 2
          ? HttpResponse.json({ detail: "failed" }, { status: 500 })
          : HttpResponse.json(recordSnapshot(10, null, "晴"));
      }),
  ...
  it("keeps a newly created record retryable when its post-save refetch fails", async () => {
    ...
    renderLedger([
      ...
      http.put("/api/ledger/1/:date", () =>
        HttpResponse.json({ id: 9, date: "2026-07-15", daily_revenue: 10 }),
      ),
    ]);
    fireEvent.change(await screen.findByLabelText("现金"), { target: { value: "10" } });
    fillBlankLedgerAmounts();
    ...
    renderLedger([
      http.get("/api/ledger/1/:date", ({ params }) =>
        params.date === "recent"
          ? HttpResponse.json([])
          : saved
            ? HttpResponse.json(recordSnapshot(10))
            : HttpResponse.json({ detail: "not found" }, { status: 404 }),
      ),
      http.put("/api/ledger/1/:date", async () => {
        await delayed;
        saved = true;
        return HttpResponse.json(recordSnapshot(10));
      }),
    ]);
    fireEvent.change(await screen.findByLabelText("现金"), { target: { value: "10" } });
    fillBlankLedgerAmounts();
  ...
  it("binds save invalidation to the original store across a store switch", async () => {
    ...
    server.use(
      http.get("/api/stores/accessible", () =>
        HttpResponse.json([
          { id: 1, name: "One", timezone: "Europe/Berlin" },
          { id: 2, name: "Two", timezone: "Europe/Berlin" },
        ]),
      ),
      http.get("/api/income-config/:store/current", ({ params }) =>
        HttpResponse.json({
          ...singleConfig,
          store_id: Number(params.store),
          items: [{ ...singleConfig.items[0], store_id: Number(params.store) }],
        }),
      ),
      http.get("/api/database/:store/records", () =>
        HttpResponse.json({
          items: [],
          categories: [
            { id: 1, name: "现金", include_in_total: true, is_active: true, sort_order: 1 },
      ...
      http.get("/api/ledger/:store/:date", () =>
        HttpResponse.json({ detail: "not found" }, { status: 404 }),
      ),
      http.put("/api/ledger/:store/:date", async ({ request }) => {
        saveUrl = request.url;
        await delayed;
        return HttpResponse.json({ id: 1, date: "2026-07-13", daily_revenue: 1 });
      }),
    );
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false }, mutations: { retry: false } },
    ...
    fireEvent.change(await screen.findByLabelText("现金"), { target: { value: "1" } });
    fillBlankLedgerAmounts();
    fireEvent.click(screen.getByRole("button", { name: "补记历史记录" }));
    client.setQueryData(["dashboard", 1], true);
    client.setQueryData(["dashboard", 2], true);
    fireEvent.click(screen.getByRole("button", { name: "choose2" }));
    fireEvent.click(await screen.findByRole("button", { name: "放弃修改" }));
    release();
    await waitFor(() => expect(saveUrl).toContain("/api/ledger/1/2026-07-13"));
    expect(new URL(saveUrl).search).toBe("");
    await waitFor(() => expect(client.getQueryState(["dashboard", 1])?.isInvalidated).toBe(true));
    expect(client.getQueryState(["dashboard", 2])?.isInvalidated).toBe(false);
  });
