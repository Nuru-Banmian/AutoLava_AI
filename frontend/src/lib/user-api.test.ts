...
describe("user API helpers", () => {
  it("invalidates only exact store positions and not a recent-days collision", async () => {
    ...
    const keys = [
      ...
      ["ledgerMonth", 7, "2026-07"], ["ledgerMonth", 8, "2026-07"], ["database", "records", 7, "page=1"], ["database", "history", 7], ["charts", 7, "start=x"],
    ...
    for (const key of [keys[1], keys[2], keys[3], keys[5], keys[6], keys[7], keys[8], keys[9]]) expect(client.getQueryState(key)?.isInvalidated).toBe(true);
