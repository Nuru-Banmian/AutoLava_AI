...
describe("user API helpers", () => {
  it("invalidates only exact store positions and not a recent-days collision", async () => {
    ...
    const keys = [
      ["ledger", "recent", 1, 7],
      ["ledger", "recent", 7, 7],
      ["ledger", "record", 7, "2026-07-14"],
      ["ledgerMonth", 7, "2026-07"],
      ["ledgerMonth", 8, "2026-07"],
      ["database", "records", 7, "page=1"],
      ["charts", 7, "start=x"],
      ["dashboard", 7],
      ["categoryCatalog", 7, "2026-07-14"],
      ["weather", 7, "2026-07-14"],
    ] as const;
    ...
    for (const key of [keys[1], keys[2], keys[3], keys[5], keys[6], keys[7], keys[8]])
