...
it("downloads exactly the active range and revokes the object URL", async () => {
  let requested = "";
  server.use(
    http.get("/api/database/7/export.xlsx", ({ request }) => {
  ...
  await downloadBusinessRecords(7, { start: "2026-07-01", end: "2026-07-31" }, "提前休息");
...
it("rejects a failed export without creating a download", async () => {
  server.use(
    http.get("/api/database/7/export.xlsx", () =>
  ...
  await expect(
    downloadBusinessRecords(7, { start: "2026-07-01", end: "2026-07-31" }),
