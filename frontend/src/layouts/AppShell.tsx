...
function Navigation({ surface }: { surface: "desktop" | "mobile" }) {
  ...
  return (
    <>
      {navigationFor(user.role, surface, selected?.company_settlement_enabled).map(
        ({ to, label, end }) => {
          const Icon = icons[to];
          return (
            <NavLink
    ...
    <div className="min-h-screen bg-muted/20 md:pl-64">
      ...
      <header className="border-b bg-background md:fixed md:left-0 md:top-0 md:z-40 md:w-64 md:border-0 md:bg-transparent md:text-primary-foreground">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          {!isAdminRoute && (
            <div data-testid="mobile-store-picker" className="ml-auto w-40 max-w-[55vw] md:hidden">
              <StorePicker showLabel={false} />
            </div>
          )}
        </div>
      </header>
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-primary p-4 text-primary-foreground md:flex">
        <div className="mt-16 grid gap-3">
          {!isAdminRoute && (
            <div
              data-testid="desktop-store-picker"
              className="min-w-0 max-w-full [&_select]:bg-background [&_select]:text-foreground"
            >
              <StorePicker />
            </div>
          )}
          <nav aria-label="主导航" className="grid gap-1">
            <Navigation surface="desktop" />
          </nav>
        </div>
      ...
      >
        ...
        {!isAdminRoute && storeError && (
          <div
            className="mb-4 flex flex-wrap items-center gap-2 text-sm text-destructive"
            role="alert"
          >
            <span>门店加载失败，请重试</span>
            <Button
              aria-label="重试门店"
              onClick={() => {
                void refetchStores();
