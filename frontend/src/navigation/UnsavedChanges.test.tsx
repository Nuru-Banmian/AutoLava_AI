...
import {
  ...
  UnsavedRouteGuard,
  useUnsavedChanges,
} from "@/navigation/UnsavedChanges";
...
function GuardHarness() {
  ...
  return (
    <>
      <UnsavedRouteGuard />
      <button onClick={() => markDirty(true)}>修改</button>
...
function BeforeUnloadHarness() {
  ...
  return (
    <>
      <button onClick={() => markDirty(true)}>设为已修改</button>
      <button onClick={() => markDirty(false)}>设为已保存</button>
    </>
  );
}

function CompetingTransitionsHarness({
  first,
  second,
  cancelSecond,
}: {
  ...
  return (
    <>
      <button onClick={() => markDirty(true)}>修改</button>
      <button onClick={() => requestTransition(first)}>第一个动作</button>
      <button onClick={() => requestTransition(second, cancelSecond)}>第二个动作</button>
    </>
  );
}
...
describe("UnsavedChangesProvider", () => {
  ...
  it("keeps one active transition and explicitly cancels a competing request", async () => {
    ...
    render(
      <UnsavedChangesProvider>
        <CompetingTransitionsHarness first={first} second={second} cancelSecond={cancelSecond} />
      </UnsavedChangesProvider>,
    );
