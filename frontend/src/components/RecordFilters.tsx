  ...
  return (
    <section aria-label="记录筛选" className="flex flex-wrap items-end gap-2">
      ...
      <label className="grid gap-1 text-sm">开始日期
        <input aria-label="开始日期" type="date" max={today} value={customDraft.start} onChange={(event) => updateCustom({ start: event.target.value })} className="rounded-md border border-input bg-background px-2 py-1.5" />
      </label>
      <label className="grid gap-1 text-sm">结束日期
        <input aria-label="结束日期" type="date" max={today} value={customDraft.end} onChange={(event) => updateCustom({ end: event.target.value })} className="rounded-md border border-input bg-background px-2 py-1.5" />
      </label>
