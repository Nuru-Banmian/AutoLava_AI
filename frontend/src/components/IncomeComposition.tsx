...
function CompositionGroup({
  ...
  return (
    <section aria-label={title} className="grid gap-3">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-muted-foreground">暂无分类金额</p>
      ) : (
        <div className="grid gap-2">
          {visibleRows.map((row) => (
            <div key={row.category_id ?? row.category_name} className="grid gap-1">
              ...
              {showProportions && (
                <div
                  data-testid="composition-proportion"
                  className="h-1.5 overflow-hidden rounded-full bg-muted"
                  aria-label={`${row.category_name} 占比 ${compositionPercentage(row.amount, total)}`}
                >
                  <div
                    className="h-full rounded-full bg-primary"
                    style={{ width: compositionPercentage(row.amount, total) }}
                  />
                </div>
              )}
              {showProportions && (
                <span className="text-xs text-muted-foreground">
                  {compositionPercentage(row.amount, total)}
                </span>
              )}
            </div>
          ))}
      ...
      {hiddenCount > 0 && (
        ...
        >
          {expanded ? `收起${title}` : `${toggleLabel}（还有 ${hiddenCount} 项）`}
        </Button>
      )}
    </section>
  );
...
export function IncomeComposition({
  ...
  return (
    <section className="grid gap-4" aria-label="收入构成">
      <CompositionGroup
        title="收入分类"
        rows={included}
