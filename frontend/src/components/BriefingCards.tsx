...
function YesterdayCard({
  ...
  card?: CardFor<"yesterday">;
  backfillHref: string;
}) {
  if (!card || card.state === "unavailable")
    return (
      <CardShell title="昨日">
        <p>昨日简报暂不可用</p>
      </CardShell>
  ...
  if (card.state === "missing")
    return (
      <CardShell title="昨日">
        <p>昨日尚未记录</p>
        <a className={buttonVariants({ variant: "outline", size: "sm" })} href={backfillHref}>
          补记昨日
        </a>
      </CardShell>
    );
  ...
  return (
    <CardShell title="昨日">
      <p>{status}</p>
      {card.revenue !== null && (
        <p className="text-2xl font-semibold">{formatWholeEuro(card.revenue)}</p>
      )}
    </CardShell>
  );
}
...
function TomorrowCard({ card }: { card?: CardFor<"tomorrow"> }) {
  ...
  return (
    <CardShell title="明日">
      ...
      {card.weather && <p>{card.weather}</p>}
      {card.temperature_min !== null && card.temperature_max !== null && (
        <p>
          {card.temperature_min}°C – {card.temperature_max}°C
        </p>
      )}
