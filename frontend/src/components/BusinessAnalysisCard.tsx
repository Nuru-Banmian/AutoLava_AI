      ...
      <CardContent className="grid gap-5">
        ...
        {charts.error && !data && (
          <div role="alert" className="flex items-center gap-3">
            <span>经营分析加载失败</span>
            <Button type="button" size="sm" variant="outline" onClick={() => void charts.refetch()}>
              重试经营分析
            </Button>
          </div>
        )}
          ...
          <>
            {data.income_summary.includes_settlement_income ? (
              ...
              >
                <Kpi
                  title="日常营业额"
                  value={formatWholeEuro(data.income_summary.daily_ledger_revenue)}
                />
                <Kpi
                  title="公司结算收入"
                  value={formatWholeEuro(data.income_summary.confirmed_settlement_income)}
                />
                <Kpi
                  title={isSingleMonth ? "月度总收入" : "月度总收入汇总"}
                  value={formatWholeEuro(data.income_summary.total_income)}
                />
              </div>
            ) : (
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                <Kpi
                  title="经营日均台账营业额"
                  value={formatWholeEuro(data.kpis.average_revenue)}
                />
              </div>
            )}
            <div className="grid gap-1 text-sm text-muted-foreground">
              <p>
                当前区间：{data.range.start} 至 {data.range.end}（按
                {data.range.bucket === "day" ? "日" : "月"}）
              </p>
              {data.comparison_kpis && (
                <p>
                  比较区间：{data.comparison_kpis.start} 至 {data.comparison_kpis.end}
                </p>
              )}
            ...
            <IncomeComposition
              ...
              excluded={data.excluded_categories}
              classifiedIncludedTotal={data.classified_included_total}
            />
          </>
        )}
      </CardContent>
