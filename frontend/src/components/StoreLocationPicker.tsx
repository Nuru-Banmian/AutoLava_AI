...
export function StoreLocationPicker({
  ...
  const resolveCoordinates = useCallback(
    async (point: Pick<MapLocation, "latitude" | "longitude">) => {
      cancelSearch();
  ...
  async function search(event: FormEvent) {
    ...
    setResults([]);
    try {
      const candidates = await api<GeocodeCandidate[]>(
        `/admin/stores/geocode?query=${encodeURIComponent(query.trim())}`,
      );
      if (sequence === searchSequence.current) setResults(candidates);
    } catch (reason) {
      if (sequence === searchSequence.current)
        setError(friendlyApiError(reason, "地点搜索失败，请稍后重试"));
    } finally {
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={close}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>选择门店位置</DialogTitle>
            <DialogDescription>
              可搜索地点、使用当前位置，或在地图中点击和拖动标记。
            </DialogDescription>
          </DialogHeader>
          <form role="search" className="flex gap-2" onSubmit={search}>
            <Input
              aria-label="搜索城市、区域或地点"
              value={query}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {draft && (
              <p className="text-sm text-muted-foreground">
                {draft.label}
                {draft.timezone ? ` · ${draft.timezone}` : " · 正在识别时区…"}
              </p>
            )}
          </div>
          {error && (
