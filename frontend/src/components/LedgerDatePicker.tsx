...
>(function DateTrigger({ value, ...props }, ref) {
  return (
    <button
      ref={ref}
      type="button"
...
function PickerPanel({
  ...
  setMonth,
  select,
}: LedgerDatePickerProps & {
  month: string;
  setMonth(month: string): void;
  select(date: string): void;
}) {
  const moveMonth = (amount: number) =>
    setMonth(format(addMonths(parseISO(`${month}-01`), amount), "yyyy-MM"));
    ...
    <div className="grid min-w-0 gap-4 pt-1">
      <div className="flex items-center justify-between gap-2 pr-8">
        <button
          ...
          aria-label="上个月"
          onClick={() => moveMonth(-1)}
          className="rounded-lg p-2 hover:bg-accent focus-visible:outline-2 focus-visible:outline-ring"
        >
        ...
        <button
          ...
          onClick={() => moveMonth(1)}
          className="rounded-lg p-2 hover:bg-accent focus-visible:outline-2 focus-visible:outline-ring disabled:opacity-30"
        >
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button
          ...
          className="rounded-lg bg-secondary px-3 py-2 text-sm font-medium text-secondary-foreground hover:bg-accent"
        >
          昨天
        </button>
      </div>
...
export function LedgerDatePicker({
  value,
  today,
  recordedDates,
  onChange,
