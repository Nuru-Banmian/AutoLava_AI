...
export function RecordTable({
  ...
  if (loading)
    return (
      <div role="status" className="animate-pulse rounded-md bg-muted p-4">
        正在加载记录…
      </div>
    );
  if (error)
  ...
  if (records.length === 0)
    return <p className="rounded-md border border-dashed p-4 text-muted-foreground">暂无记录</p>;
          ...
          {records.map((record) => {
            ...
            return (
              <tr
                ...
                className={
                  selected
                    ? "cursor-pointer border-l-4 border-primary bg-primary/10"
                    : "cursor-pointer border-l-4 border-transparent hover:bg-muted/60"
                }
              >
