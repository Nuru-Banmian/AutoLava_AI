...
function renderCard(range: DateRange = { start: "2026-07-01", end: "2026-07-17" }) {
  ...
  const view = render(
    <QueryClientProvider client={client}>
      <BusinessAnalysisCard storeId={1} range={range} />
    </QueryClientProvider>,
  );
  return { ...view, client };
}
...
describe("BusinessAnalysisCard", () => {
  it("drives all analysis content from the supplied record-table range without separate controls", async () => {
    ...
    expect(requests[0].pathname + requests[0].search).toBe(
      "/api/charts/1?start=2026-07-01&end=2026-07-17&bucket=day",
    );
  ...
  it("renders the zero-data and retry states", async () => {
    ...
    server.use(
      http.get("/api/charts/1", () => HttpResponse.json({ detail: "failed" }, { status: 500 })),
    );
