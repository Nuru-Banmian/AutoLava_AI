...
export function RecordDetailPanel({
  ...
  const showWashCount =
    !isUnrecorded &&
    washCountEnabled &&
    typeof record.wash_count === "number" &&
    record.wash_count > 0;

  return (
    <Card className={mobile ? "border-0 shadow-none" : "overflow-hidden"}>
      <CardHeader
        className={`flex-row flex-wrap items-center gap-2 space-y-0 ${mobile ? "p-1 pb-5 pr-14" : "p-5 pb-4"}`}
      >
        <CardTitle className={mobile ? "text-2xl leading-tight" : "text-xl leading-tight"}>
          {format(parseISO(record.date), "yyyy年M月d日")}
        </CardTitle>
        <span
          className={`rounded-full px-2.5 py-1 text-sm font-medium ${isUnrecorded ? "bg-muted text-muted-foreground" : statusClasses[record.is_open]}`}
        >
          {status}
        </span>
      </CardHeader>
      <CardContent className={mobile ? "grid gap-5 p-1 pt-0" : "grid gap-5 p-5 pt-0"}>
        <section aria-label="营业摘要" className="grid grid-cols-2 gap-3 text-sm">
          <div className={summaryItemClass}>
            ...
            <p className={summaryValueClass}>
              {isUnrecorded ? "—" : formatWholeEuro(record.daily_revenue)}
            </p>
          </div>
          <div className={summaryItemClass}>
            <p className="text-muted-foreground">天气</p>
            <p className={summaryValueClass}>{isUnrecorded ? "—" : (record.weather ?? "—")}</p>
          </div>
        </section>

        {showWashCount && (
          <p className="text-sm font-medium text-muted-foreground">洗车 {record.wash_count} 辆</p>
        )}
        ...
        {!isUnrecorded && record.income_mode === "legacy_total" ? (
          ...
          <section className="grid gap-2" aria-labelledby={`income-details-${record.date}`}>
            <h4 className="font-semibold" id={`income-details-${record.date}`}>
              收入明细
            </h4>
            <dl
              aria-label="收入明细"
              className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,9rem),1fr))] gap-2 text-base"
            >
              {composedItems.map((item) => (
                <div
                  key={item.id}
                  className="flex min-w-0 items-start justify-between gap-3 rounded-lg bg-muted/50 px-3 py-2.5"
                >
        ...
        <div className={`flex flex-wrap gap-2 border-t pt-4 ${mobile ? "" : "items-center"}`}>
          {canEdit && (
            <Button asChild className={mobile ? "h-11 w-full text-base" : "h-10 text-base"}>
              <Link
                to={`/ledger?date=${record.date}`}
                onClick={
                  onEdit
                    ? (event) => {
          ...
          {canDelete && !isUnrecorded && (
            <Button
              className={mobile ? "h-11 w-full text-base" : "h-10 text-base"}
              type="button"
              variant="destructive"
