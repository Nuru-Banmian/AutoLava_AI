...
export function ChartPanel({
  ...
  const tooltip = (
    <Tooltip
      formatter={(_value, _name, item) =>
        chartTooltipValue(item.payload as Record<string, unknown>, valueKey!)
      }
    />
  );
  const header = (
  ...
  const content = (
    <CardContent data-testid="chart-panel-content">
      {children ??
        (!rows.length ? (
          ...
          <div data-testid="chart-panel-plot" className={`${heightClassName} w-full`}>
            <ResponsiveContainer width="100%" height="100%">
              {kind === "line" ? (
                ...
                <BarChart data={rows} layout="vertical">
                  ...
                  {tooltip}
                  <Bar dataKey={valueKey!} fill={chartSeriesColors[1]} radius={[0, 6, 6, 0]} />
                </BarChart>
              ) : (
                <BarChart data={rows}>
                  <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" />
                  ...
                  <YAxis />
                  {tooltip}
                  <Bar dataKey={valueKey!} fill={chartSeriesColors[0]} />
                </BarChart>
              )}
            </ResponsiveContainer>
          </div>
