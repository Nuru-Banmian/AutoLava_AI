...
export function LedgerForm({ categories, record, weather, onSave, saving = false }: { categories: CategoryDescriptor[]; record?: RecordSnapshot; weather?: WeatherResponse; onSave(body: LedgerBody): void; saving?: boolean }) {
  ...
  return <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); const items = active.filter((c) => c.is_active || record?.items.some((i) => i.category_id === c.id)).map((category) => ({ category_id: category.id, result: status === "休息" ? { value: "0.00" } : canonicalAmount(amounts[category.id] ?? "") })); const invalid = items.map((item) => item.result).find((result): result is { error: string } => "error" in result); if (invalid) { setValidationError(invalid.error); return; } setValidationError(""); onSave({ is_open: status, daily_revenue: null, config_version_id: record?.income_config_version_id ?? null, expected_version: record?.row_version ?? null, wash_count: status === "休息" ? 0 : wash === "" ? null : Number(wash), weather: weatherValue || null, weather_edited: weatherEdited, activity: activity.trim() || null, items: items.map((item) => ({ category_id: item.category_id, amount: "value" in item.result ? item.result.value : "0.00" })) }); }}>
    ...
    <p className="text-xl font-semibold">合计 {total === null ? "—" : centsToMoney(total)}</p>{validationError && <p role="alert">{validationError}</p>}<Button disabled={saving} type="submit">保存</Button>
  </form>;
}
