...
const MANUAL_RECORD_WEATHER_OPTIONS = [
  "晴",
  "少云",
  "多云",
  "阴",
...
const LEDGER_FIELD_CLASS =
  "min-h-11 w-full min-w-0 rounded-md border border-input bg-background px-3 py-2 text-base shadow-sm outline-none transition-colors focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50";
...
export function LedgerForm({
  ...
  const incomingSignature = JSON.stringify({
    ...
    recordRevision,
    loadedAmounts,
    automaticWeather: record ? null : (weather?.weather ?? null),
  });
  ...
  const semanticSignature = (values: {
    status: LedgerStatus;
    wash: string;
    weatherValue: string;
    weatherEdited: boolean;
    activity: string;
    directTotal: string;
  ...
  const submittedSignature = (body: LedgerBody) =>
    JSON.stringify({
  ...
  const loadedSemanticSignature = semanticSignature({
    status: record?.is_open ?? "营业",
    wash: loadedWash,
    weatherValue: record?.weather ?? weather?.weather ?? "",
    weatherEdited: record?.weather_edited ?? false,
    activity: record?.activity ?? "",
    directTotal: loadedDirectTotal,
  ...
  const currentSignature = semanticSignature({
    status,
    wash,
    weatherValue,
    weatherEdited,
  ...
  const pendingSavedSubmission =
    savedSubmission?.revision === consumedSubmissionRevision ? undefined : savedSubmission;
  const effectiveBaselineSignature = pendingSavedSubmission
    ? submittedSignature(pendingSavedSubmission.body)
    : baselineSignature;
  ...
  useEffect(() => {
    ...
    setStatus(record?.is_open ?? "营业");
    setWash(loadedWash);
    setWeatherValue(record?.weather ?? weather?.weather ?? "");
    setWeatherEdited(record?.weather_edited ?? false);
    ...
    appliedIncomingSignature,
    canonicalSavedRecordReady,
    currentSignature,
    effectiveBaselineSignature,
    incomingSignature,
    loadedAmounts,
    loadedDirectTotal,
    loadedSemanticSignature,
    loadedWash,
    pendingSavedSubmission,
    record,
    weather?.weather,
  ]);
  useEffect(() => {
  ...
  const includedAmounts = amountResults
    ...
    .map(({ result }) => result);
  const calculatedTotal = includedAmounts.every(
    (result): result is { value: number } => "value" in result,
  )
    ...
    status === "休息"
      ...
      : composed
        ...
        : "error" in directResult
          ? { category: { name: "当日营业额" }, result: directResult }
          : undefined;
  const amountError =
    invalidAmount && "error" in invalidAmount.result
  ...
  return (
    ...
    >
      <section role="group" aria-label="状态与天气" className="grid min-w-0 gap-4 md:grid-cols-2">
        <label className="grid min-w-0 gap-1.5 font-medium">
          状态
          <select
            aria-label="状态"
            value={status}
            onChange={(event) => changeStatus(event.target.value as LedgerStatus)}
            className={LEDGER_FIELD_CLASS}
          >
            <option>营业</option>
            <option>休息</option>
            <option>提前休息</option>
        ...
        <div className="grid min-w-0 gap-1.5">
          ...
          >
            <SelectTrigger aria-label="天气" className="h-11 text-base">
              <SelectValue placeholder="请选择天气">{weatherValue || undefined}</SelectValue>
            </SelectTrigger>
            <SelectContent>
              {MANUAL_RECORD_WEATHER_OPTIONS.map((value) => (
            ...
            {active.map((category) => (
              <label className="grid min-w-0 gap-1.5 font-medium" key={category.id}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setAmounts((old) => ({ ...old, [category.id]: event.target.value }))
                  }
                  className={LEDGER_FIELD_CLASS}
                />
              </label>
      ...
      <section
        className={`grid min-w-0 gap-4 rounded-lg border bg-background p-4 ${washCountEnabled ? "md:grid-cols-2" : ""}`}
      >
        {washCountEnabled && (
          <label className="grid min-w-0 gap-1.5 font-medium">
            洗车数量
            <input
              ...
              disabled={status === "休息"}
              value={wash}
              onChange={(event) => setWash(event.target.value)}
              className={LEDGER_FIELD_CLASS}
            />
          </label>
        )}
        <label className="grid min-w-0 gap-1.5 font-medium">
          ...
          <textarea
            aria-label="事件"
            placeholder="记录可能影响经营的特殊情况，如当地活动、泥雨等（选填）"
            value={activity}
            onChange={(event) => setActivity(event.target.value)}
            className={`${LEDGER_FIELD_CLASS} resize-y`}
          />
        </label>
      </section>
      {validationError && (
      ...
      <footer className="flex min-w-0 flex-col gap-4 rounded-lg bg-muted/50 p-4 sm:flex-row sm:items-center sm:justify-between">
        {composed && (
          <p className="text-xl font-semibold tabular-nums">
            合计金额 {formatWholeEuro(status === "休息" ? 0 : latestValidTotal)}
          </p>
        )}
        <Button
          className="min-h-11 w-full px-6 text-base sm:ml-auto sm:w-auto sm:min-w-40"
          disabled={saving}
          type="submit"
        >
          {submitLabel}
